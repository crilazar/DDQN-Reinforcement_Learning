...
class DDQNAgent(object):
    def __init__(self, alpha, gamma, n_actions, epsilon, batch_size,
                 ...
                 mem_size=3000000, fname='ddqn_forex-b256-nn128_64-lr0_0005-env5.h5', replace_target=400):
        ...
        self.q_target = build_dqn(alpha, n_actions, input_dims, 128, 64)
...
import numpy as np
from env.forex_env_v5 import Forex1 as ForexTrading
...
def write_to_log(message):
    with open("out-ddqn_forex-b256-nn128_64-lr0_0005-env5.log", "a") as file:
...
if __name__ == '__main__':
    ...
    ddqn_agent = DDQNAgent(alpha=0.0005, gamma=0.99, n_actions=4, epsilon_dec=0.999983, epsilon=1.0, batch_size=256, input_dims=22)
    n_games = 1000    
    ddqn_scores = []
    eps_history = []
    ...
    if path.exists("ddqn_forex-b256-nn128_64-lr0_0005-env5.h5"):
    ...
    for i in range(n_games):
        ...
        write_to_log(f'episode: {i} score: {int(score)} average score {int(avg_score)} time {time_to_print} balance {int(info[0])} buy {info[1]}/{info[2]} sell {info[3]}/{info[4]} pips_won {int(info[7])} pips_lost {int(info[8])} eps {eps_history[len(eps_history)-1]} avg_length {int(info[9])} max_length {int(info[10])} avg_profit {int(info[11])} avg_loss {int(info[12])}')

        #if i % 2 == 0 and i > 0:
        ddqn_agent.save_model()
        source_save_file = 'ddqn_forex-b256-nn128_64-lr0_0005-env5.h5'
